...
namespace POS.Sale
{
    public partial class frm_SaleList : Form
    {
        ...
        private void ShowSale()
        {
            ...
            dgvSale.Columns[1].Width = (dgvSale.Width / 100) * 8;
        ...
        private void dgvSale_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                ...
                if (dgvSale[e.ColumnIndex, e.RowIndex].Value.ToString() == "+")
                {
        ...
        private void tstSearchWith_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                SPString = string.Format("SP_Select_Sale N'{0}', N'{1}', N'{2}'", tstSearchWith.Text.Trim().ToString(), "0", "4");
            }
            else if (tslLabel.Text == "UserName")
            {
                SPString = string.Format("SP_Select_Sale N'{0}', N'{1}', N'{2}'", tstSearchWith.Text.Trim().ToString(), "0", "5");
            }
            dgvSale.DataSource = obj_clsMainDB.SelectData(SPString);
        }
    }
