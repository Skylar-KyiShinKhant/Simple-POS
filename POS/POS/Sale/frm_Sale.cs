...
namespace POS.Sale
{
    public partial class frm_Sale : Form
    {
        ...
        private void CreateTable()
        {
            ...
            dgvSale.Columns[1].Width = (dgvSale.Width / 100) * 25;
            dgvSale.Columns[2].Width = (dgvSale.Width / 100) * 25;
            dgvSale.Columns[2].ReadOnly = true;
            dgvSale.Columns[3].Width = (dgvSale.Width / 100) * 20;
            dgvSale.Columns[3].ReadOnly = true;
            dgvSale.Columns[4].Width = (dgvSale.Width / 100) * 18;
        ...
        private void dgvSale_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (ItemName != "")
            {
                if (CurCol - 1 == 1)
                {
                    ...
                    if (DT.Rows.Count <= 0)
                    {
                        MessageBox.Show("This ItemName Does Not Exist", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        SendKeys.Send("{HOME}");
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < dgvSale.Rows.Count - 1; i++)
                        {
                            ...
                            {
                                MessageBox.Show("This Item Has Already Sold", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                AddRow = false;
                                SendKeys.Send("{HOME}");
                            }
                        }
                ...
                if (CurCol - 1 == 4)
                {
                    ...
                    {
                        MessageBox.Show("SaleQty Should Be Number", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        SendKeys.Send("{HOME}");
                        SendKeys.Send("{TAB}");
                    }
                    ...
                    {
                        MessageBox.Show("SaleQty Should Be Between 1 And " + dgvSale.CurrentRow.Cells["QtyOnHand"].Value.ToString(), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        SendKeys.Send("{HOME}");
                        SendKeys.Send("{TAB}");
                    }
                    else
        ...
        private void mnuPayment_Click_1(object sender, EventArgs e)
        {
            if (lblGrandTotal.Text == "")
            {
                MessageBox.Show("There Is No Item Record", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                dgvSale.Focus();
            }
            else
            {
        ...
        private void txtPayment_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            else
            {
                ...
                if (e.KeyChar.Equals((char)Keys.Enter))
                {
                    if (txtPayment.Text == "")
                    {
                        MessageBox.Show("Please Type Payment", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtPayment.Focus();
                    }
                    else if (int.TryParse(txtPayment.Text, out OK) == false)
                    {
                        MessageBox.Show("Payment Should Be Number", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtPayment.Focus();
                        txtPayment.SelectAll();
                    }
                    else if (Convert.ToInt32(txtPayment.Text) < Convert.ToInt32(lblGrandTotal.Text))
                    {
                        MessageBox.Show("Payment Amount Should Be Above " + lblGrandTotal.Text, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtPayment.Focus();
                        txtPayment.SelectAll();
                    }
                    else if (Convert.ToInt32(txtPayment.Text) > Convert.ToInt32(lblGrandTotal.Text) + 10000)
                    {
                        MessageBox.Show("Payment Amount Should Be Below " + (Convert.ToInt32(lblGrandTotal.Text) + 10000), "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        txtPayment.Focus();
                        txtPayment.SelectAll();
                    }
                    else
        ...
        private void dtpDate_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (DateDiff > 0)
            {
                MessageBox.Show("Please Check SaleDate", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void mnuSave_Click_1(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please Check Payment", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtPayment.Focus();
            }
            else
            {
